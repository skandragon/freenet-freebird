---
type: spec
title: The dual-read window
description: Rotating a contract's wasm mints a new address and orphans the state at the old one; the window is the period a build reads both generations. Covers the _v1 blob invariant, per-role terminators, and why directory and inbox windows cannot close on a date.
