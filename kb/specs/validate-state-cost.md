---
type: spec
title: Full-state validate_state cost stays uncached
description: Why unbounded RSA attestation verification in the full-state path cannot be cached or weakened — no cross-call wasm memory, no held-state context, and it is the sole guard on fresh adoption. Issues #52, #70, #71. Records the measured per-entry cost so the analysis is not redone.
timestamp: 2026-08-14T00:00:00Z
status: living
covers:
  - contracts/directory-contract/src/lib.rs
  - contracts/inbox-contract/src/state.rs
  - common/src/attestation.rs
---
...
This lives in the vault and not as comments in the contract
crates: contract source is byte-frozen between rotations (any wasm byte
change rotates the address — see [reproducible builds](../policies/reproducible-builds.md) — and CI
