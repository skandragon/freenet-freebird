---
type: policy
title: Reproducible contract and delegate builds
description: The vendored wasm in ui/contracts/ is the immutable source of truth; amd64 is the single build-of-record. Covers why a plain build is not reproducible, the no-whitespace-churn rule for contract crates, the step-by-step rotation procedure, and building on a native amd64 host when Apple Silicon cannot.
timestamp: 2026-08-14T00:00:00Z
covers:
  - Makefile
  - docker/Dockerfile
  - scripts/repro-reference-hashes.txt
...
— see [the dual-read window](../specs/dual-read-window.md) and `make check-legacy-wasm`.
...
5. Add a dual-read window so existing data stays reachable, and follow
   [the dual-read window](../specs/dual-read-window.md) — re-vendor the `_v1` blob from
...
`docker/Dockerfile` pins. See [anonymous-PoW difficulty](../specs/anon-pow-difficulty.md) for the worked
example.

### Identifying who built a wasm

The vendored set is mixed-provenance while the grandfathered bytes last, so it
is sometimes necessary to ask which toolchain and which account produced a
given blob. Both are readable straight out of the bytes:

```bash
strings x.wasm | grep -oE '/rustc/[0-9a-f]{40}'   # rustc commit hash
strings x.wasm | grep .cargo/registry             # build account (unremapped builds)
```

Match the rustc hash against
`git ls-remote https://github.com/rust-lang/rust 'refs/tags/1.9*' | grep '\^{}'`.
A blob built through `wasm-repro` shows the remapped `/rust` and `/cargo`
prefixes instead — their absence is itself the tell that a blob predates the
reproducible path.
